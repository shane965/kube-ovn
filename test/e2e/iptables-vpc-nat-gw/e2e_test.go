...
var _ = framework.Describe("[group:iptables-vpc-nat-gw]", func() {
	...
	net2AttachDefName = "ovn-vpc-external-network-" + framework.RandomSuffix()
	net2SubnetProvider = net2AttachDefName + ".kube-system"
	net2overlaySubnetName = "overlay-subnet-" + framework.RandomSuffix()
	net2vpcNatGwName = "gw-" + framework.RandomSuffix()
	net2vpcName = "vpc-" + framework.RandomSuffix()
	...
	framework.ConformanceIt("iptables eip fip snat dnat", func() {
		...
		setupVpcNatGwTestEnvironment(
			...
			overlaySubnetV4Cidr, "192.168.0.1", "192.168.0.254",
		...
		setupVpcNatGwTestEnvironment(
			...
			net2overlaySubnetV4Cidr, "192.168.1.1", "192.168.1.254",
			dockerExtNet2Name, net2AttachDefName, net2NicName,
			net2SubnetProvider)

		ginkgo.By("Creating iptables eip of net2")
		net2tEip := framework.MakeIptablesEIP(net2EipName, "", "", "", net2vpcNatGwName, net2AttachDefName)
		_ = iptablesEIPClient.CreateSync(net2tEip)
